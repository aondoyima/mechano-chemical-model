...
The other fields that characterise the material are local velocity ${\bf{v}}({\bf{r}}, t)$ and the local passive stress $\boldsymbol{\pi}({\bf{r}}, t)$. The total stress $\boldsymbol{\sigma}({\bf{r}}, t)$ in the material is the sum of the passive stress and an active stress proportional to $\mathbf{M}$ : $\mathbf{\sigma} = \boldsymbol{\pi} + \beta(\mathbf{M} - m_0\mathbf{I})$, where $\beta$ is the activity parameter and $m_0$ is the reference concentration for actomyosin. The actomyosin tensor evolves via
```math
